...
public class OpenTsdbTimeSeriesDAO
	extends DaoBase
	implements TimeSeriesDAI
{
	...
	public OpenTsdbTimeSeriesDAO(DatabaseConnectionOwner tsdb)
	{
					...
					if (VarFlags.mustWrite(tv2write))
					{
						q = "update " + tableName + " set  ts_value = "
							+ (ctsid.getStorageType() == 'N' ? tv2write.getDoubleValue()
								: sqlString(tv2write.getStringValue()))
							+ ", flags = " + flags
							+ ", source_id = " + tv2write.getSourceId()
		...
		throws DbIoException, NoSuchObjectException, BadTimeSeriesException
	{
		...
		String q = "insert into TS_SPEC(" + ts_columns + ") values "
			+ key + ", "
			+ siteId + ", "
			+ dataTypeId + ", "
			+ ctsid.getStatisticsCode() + ", "
			+ intervalId + ", "
			+ durationId + ", "
			+ ctsid.getVersion() + ", "
