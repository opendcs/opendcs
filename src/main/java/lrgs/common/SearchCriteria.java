...
*
* $Log$
* Revision 1.13  2013/03/02 17:30:48  mmaloney
* removed debugs.
*
* Revision 1.12  2013/02/28 16:34:18  mmaloney
* New sources added.
*
* Revision 1.11  2011/05/24 14:21:32  mmaloney
* DapsStatus may be A, Y, R, N, or O
*
* Revision 1.10  2010/04/14 19:37:52  mjmaloney
* Implemented SINGLE: true searchcrit so that dbedit LoadMessageDialog can get a single message, but still use the extended XML protocol mode.
*
* Revision 1.9  2009/03/11 18:30:01  mjmaloney
* iridium development
*
* Revision 1.8  2008/09/26 14:57:31  mjmaloney
* Added <all> and <production> network lists
*
* Revision 1.7  2008/08/19 16:38:15  mjmaloney
* DcpAddress stores internal value as String.
*
* Revision 1.6  2008/07/07 12:03:03  mjmaloney
* dev
*
*/
...
public class SearchCriteria implements Serializable
{
	...
	*/
	public Date evaluateLrgsSinceTime() 
		throws SearchSyntaxException
	{
		if (LrgsSince == null)
			return null;
		try { return IDateFormat.parse(LrgsSince); }
		catch(Exception ex)
		{
			throw new SearchSyntaxException(ex.getMessage(),
	...
	*/
	...
	/**
	  ...
	  expressed as a relative time to 'now'.
	*/
	public Date evaluateLrgsUntilTime() 
		throws SearchSyntaxException
	{
		if (LrgsUntil == null)
			return null;
		try { return IDateFormat.parse(LrgsUntil); }
		catch(Exception ex)
		{
			throw new SearchSyntaxException(ex.getMessage(),
				LrgsErrorCode.DBADUNTIL);
		}
	}
	...
	*/
	...
	/**
	  ...
	  expressed as a relative time to 'now'.
	*/
	public Date evaluateDapsSinceTime() 
		throws SearchSyntaxException
	{
		if (DapsSince == null)
			return null;
		try { return IDateFormat.parse(DapsSince); }
		catch(Exception ex)
		{
			throw new SearchSyntaxException(ex.getMessage(),
				LrgsErrorCode.DBADSINCE);
	...
	*/
	...
	/**
	  ...
	  expressed as a relative time to 'now'.
	*/
	public Date evaluateDapsUntilTime() 
		throws SearchSyntaxException
	{
		if (DapsUntil == null)
			return null;
		try { return IDateFormat.parse(DapsUntil); } 
		catch(Exception ex)
		{
			throw new SearchSyntaxException(ex.getMessage(),
				LrgsErrorCode.DBADUNTIL);
	...
	*/
	public boolean parseFile(File input) 
		throws IOException, SearchSyntaxException
	{
		clear();
		return parseFile(new FileReader(input));
	}
		...
		throws IOException, SearchSyntaxException
	{
					...
					if (hidx != -1)
					{
						...
						catch(Exception ex)
						{
							throw new SearchSyntaxException("Bad channel range '"
								+ t + "'", LrgsErrorCode.DBADSEARCHCRIT);
						}
					}
					else
						addChannelToken(t);
				...
				else if (kw.equalsIgnoreCase("SEQUENCE:"))
				{
					if (!st.hasMoreTokens())
						throw new SearchSyntaxException("Expected sequence start.",
							LrgsErrorCode.DBADSEARCHCRIT);
					String t = st.nextToken();
					try { seqStart = Integer.parseInt(t); }
		...
		catch(SearchSyntaxException synex) { throw synex; }
		catch(Exception ex)
		{
			// numberformat, etc.
			throw new SearchSyntaxException(
