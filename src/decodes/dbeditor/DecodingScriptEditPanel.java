...
public class DecodingScriptEditPanel 
	extends JPanel
...
//	private static StringBufferConsumer consumer = new StringBufferConsumer(new StringBuffer());
	...
	/** Noargs constructor */
	public DecodingScriptEditPanel()
	{
		origScript = theScript = null;
		
		unitConversionTable = new JTable(new UnitConversionTableModel(null, this));
		
//		TableColumnAdjuster.adjustColumnWidths(formatStatementTable, new int[]
//		{ 7, 93 });
...
//System.out.println("No decoded sample at row " + row + ", col " + col);
	...
	{ // Allow multiple deletes
		...
		// FormatStatement fs;
		// if (r == -1 ||
		// (fs = formatStatementTableModel.getObjectAt(r)) == null)
		// {
		// TopFrame.instance().showError(
		// dbeditLabels.getString("DecodingScriptEditPanel.selectDelete"));
		// return;
...
class FormatStatementTableModel extends AbstractTableModel
{
	...
	{ DecodingScriptEditPanel.dbeditLabels.getString("DecodingScriptEditPanel.labelCol"),
		DecodingScriptEditPanel.dbeditLabels.getString("DecodingScriptEditPanel.formatCol") };
...
{
	static String columnNames[] =
	{ "#", DecodingScriptEditPanel.genericLabels.getString("name"),
		DecodingScriptEditPanel.genericLabels.getString("units"),
		DecodingScriptEditPanel.genericLabels.getString("algorithm"), "A", "B", "C", "D", "E", "F" };
	private DecodesScript theScript;
	private DecodingScriptEditPanel parent = null;

	public UnitConversionTableModel(DecodesScript ds, DecodingScriptEditPanel parent)
	{
		setScript(ds);
		this.parent = parent;
	}
		...
		if (parent.decodingDone)
		{
			...
			if (decodedSensorColumn >= 0) // this sensor has decoded data
			{
				int colorIdx = decodedSensorColumn % 
					DecodingScriptEditPanel.sensorColorHtml.length;
				htmlColor = DecodingScriptEditPanel.sensorColorHtml[colorIdx];                        
			}
		}
...
//			int oldn = this.getNumCoeffs(uc.algorithm);
		...
		else
		{
			...
			{
				TopFrame.instance().showError(
					DecodingScriptEditPanel.dbeditLabels
...
class DecodedDataTableModel extends AbstractTableModel
{
	...
	public void setDecodedData(DecodedMessage decmsg)
	{
			...
			numTS++)
		{
			...
			columnColor[numTS] = DecodingScriptEditPanel.sensorColorHtml[
			     numTS % DecodingScriptEditPanel.sensorColorHtml.length];
		}
