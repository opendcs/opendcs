...
/**
 * This frame contains all components used by the Run Computations GUI.
 * 
 */
public class RunComputationsFrame extends TopFrame
{
	private static ResourceBundle labels = null;
	private static ResourceBundle genericLabels = null;
	private String frameTitle;
	private String selectCompRunTitle;
	public static String computationName;
...
//TODO
	// I don't think traceLogger should be static or even an instance variable.
	// Rather it should be declared, used, and discarded inside the method that does
	// the run.
//	public static TraceLogger traceLogger = null;
	// Trace dialog can be an instance variable with lazy initialization.
	// Initialize the first time runButtonPressed() is called.
	private TraceDialog traceDialog = null;
	// Create a JButton "Trace" but have it disabled. Put it right under the
	// Run Computation button.
	
	/**
	 * Constructor
	 * @param standAloneMode True if running from launcher or tester. False if running inside compedit.
	 */
	public RunComputationsFrame(boolean standAloneMode)
	{
		super();
	...
	{
		frameTitle = 
			labels.getString("RunComputationsFrame.frameTitle");
		selectCompRunTitle = 
			labels.getString("RunComputationsFrame.selectCompRunTitle");
		computationName = 
			labels.getString("RunComputationsFrame.computationName");
		description = 
			labels.getString("RunComputationsFrame.description");
		selectFromListLabel = 
			labels.getString("RunComputationsFrame.selectFromListLabel");
		removeFromListLabel = 
			labels.getString("RunComputationsFrame.removeFromListLabel");
		selectButtonLabel = 
			labels.getString("RunComputationsFrame.selectButtonLabel");
		selectTimeRun = 
			labels.getString("RunComputationsFrame.selectTimeRun");
		fromLabel = 
			labels.getString("RunComputationsFrame.fromLabel");
		toLabel = 
			labels.getString("RunComputationsFrame.toLabel");
		chartXLabel = 
			labels.getString("RunComputationsFrame.chartXLabel");
		runCompsButton = 
			labels.getString("RunComputationsFrame.runCompsButton");
		saveOutputButton = 
			labels.getString("RunComputationsFrame.saveOutputButton");
		timeLabel = 
			labels.getString("RunComputationsFrame.timeLabel");
		noCompSelectedErr = 
			labels.getString("RunComputationsFrame.noCompSelectedErr");
		closeButtonLabel = genericLabels.getString("close");
		saveCompOutput = 
			labels.getString("RunComputationsFrame.saveCompOutput");
		okButtonLabel = genericLabels.getString("OK");
		cancelButtonLabel = genericLabels.getString("cancel");
		dateTimeColumnLabel = 
			labels.getString("TimeSeriesTable.dateTimeColumnLabel") + " (" + timeZoneStr + ")";
		inputLabel = 
			labels.getString("RunComputationsFrame.inputLabel");
		outputLabel = 
			labels.getString("RunComputationsFrame.outputLabel");
	}

	/**
	 * When lauch from Comp Edit GUI need to set the parent
	 * so that we can get the DbComputation obj when the user
	 * presses Run Computation button.
	 * 
	 * @param compEdit the parent of this frame
	 */
	...
	private JPanel listPanel()
	{
		...
		list.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED),
				selectCompRunTitle));
		...
		listButtonPanel.add(selectButton, 
				new GridBagConstraints(0,0,1,1,0,0,
						GridBagConstraints.NORTH,GridBagConstraints.HORIZONTAL,
						new Insets(4,10,0,10),0,0));
		listButtonPanel.add(removeButton, new GridBagConstraints(0,1,1,1,0,1,
				GridBagConstraints.NORTH,GridBagConstraints.HORIZONTAL,
				new Insets(4,10,0,10),0,0));
	...
	private JPanel timePanel()
	{
		...
		time.setBorder(new TitledBorder(new EtchedBorder(EtchedBorder.LOWERED),
				selectTimeRun));
		time.setLayout(new BorderLayout());

		JPanel timehalf = new JPanel();
		timehalf.setLayout(new GridBagLayout());
		timehalf.add(fromDTCal, new GridBagConstraints(0,0,1,1,0.5,0,
				GridBagConstraints.CENTER,GridBagConstraints.HORIZONTAL,
				new Insets(4,10,4,10),0,0));
		timehalf.add(new JLabel(" (" + timeZoneStr + ")"), 
			new GridBagConstraints(1,0,1,1,0,0,
				GridBagConstraints.WEST,GridBagConstraints.NONE,
				new Insets(4,10,4,10),0,0));
		timehalf.add(toDTCal, new GridBagConstraints(0,1,1,1,0.5,0,
				GridBagConstraints.CENTER,GridBagConstraints.HORIZONTAL,
				new Insets(4,10,4,10),0,0));
		...
		runhalf.add(runButton,new GridBagConstraints(0,0,1,1,0,0,
				GridBagConstraints.CENTER,GridBagConstraints.HORIZONTAL,
				new Insets(4,10,0,10),0,0));
		runhalf.add(saveButton,new GridBagConstraints(1,0,1,1,0,0,
				GridBagConstraints.CENTER,GridBagConstraints.HORIZONTAL,
				new Insets(4,10,0,10),0,0));
		runhalf.add(traceButton,new GridBagConstraints(0,1,1,1,0,0,
			GridBagConstraints.CENTER,GridBagConstraints.HORIZONTAL,
			new Insets(4,10,0,10),0,0));
	...
	{
		mychart = ChartFactory.createTimeSeriesChart("", timeLabel,
				" ",// Name of the site plus timeseries + units
				null, // dateset
				true, // legend
				true, // tooltip
				false // url
	...
	{	
		...
		mychart = ChartFactory.createTimeSeriesChart("", chartXLabel,
				correctedTs.getUnitsAbbr(),//Main timeseries units
				null, // dataset
				true, // legend
				true, // tooltip
				false // url
		...
		mychart.getXYPlot().setDatasetRenderingOrder(
				DatasetRenderingOrder.FORWARD);
		mychart.getXYPlot().setSeriesRenderingOrder(
		...
		for (int i = 0; i < ctsList.size(); i++)
		{
					...
					i == 0)
			{
				...
				xyDatasets[i] = createDataset(i, timeSeriesName
						+ " ("+tempTs.getUnitsAbbr()+")",tempTs, false);
				mychart.getXYPlot().setDataset(xyDatasets[i]); // Y left axis

				XYPlot plot = mychart.getXYPlot();
				plot.getRangeAxis().setLabel(
						" ( " + (tempTs.getUnitsAbbr()).toUpperCase() + " )");
				XYLineAndShapeRenderer rr = null;
				XYItemRenderer renderer = plot.getRenderer();
				SimpleDateFormat df1 = 
					new SimpleDateFormat("ddMMMyyyy HH:mm");			
			...
			else if (correctedTs.getUnitsAbbr().equals(tempTs.getUnitsAbbr()) 
					&& i > 0)
			{
				xyDatasets[0] = createDataset(0, timeSeriesName
			...
			{   //Y right axis, here we can add more Y right axis
				...
				Integer indexOfEngUnits = 
					(Integer)dataHashMap.get(tempTs.getUnitsAbbr());
				if (indexOfEngUnits == null)
				{	//new eng units
					dataHashMap.put(tempTs.getUnitsAbbr(), i);
					xyDatasets[i] = 
						createDataset(i, timeSeriesName
								+ " ("+tempTs.getUnitsAbbr()+")", tempTs, false);
					XYPlot plot = mychart.getXYPlot();
				...
				else
				{
					...
					xyDatasets[indexForEngUnits] = 
						createDataset(indexForEngUnits, 
								timeSeriesName
								+ " ("+tempTs.getUnitsAbbr()+")", tempTs, true);
					mychart.getXYPlot().setDataset(
							indexForEngUnits, xyDatasets[indexForEngUnits]);
				}
			}
		}
		XYPlot plot = mychart.getXYPlot();
		plot.clearRangeMarkers(); // clear all previous markers
		//Here we are going to add the markers comming from 
		//the Limit class.
		//setDomainXAxisLabels(plot, correctedTs);
	 ...
	 * @return
	 */
	private XYDataset createDataset(int index, String timeSeriesName,
			CTimeSeries tsIn,
			boolean sameEngUnits)
	{
		...
		for(int i=0; i<tsIn.size(); i++)
		{
			...
			{	//get flag and verify if value rejected, if rejected
				...
				series.addOrUpdate(
						new Minute(timestamp, timezone), value);	
	...
	/**
	 * Create the X axis "Domain Axis" according to the first time
	 * and last time of the CTimeSeries.
	 * 
	 * @param plot
	 * @param ts
	 */
	public void setDomainXAxisLabels(XYPlot plot, CTimeSeries ts)
	{
		...
		if (timeRange <= hourRange)
		{
			...
			info[0] = new PeriodAxisLabelInfo(Hour.class, 
					...
					PeriodAxisLabelInfo.DEFAULT_FONT, 
					PeriodAxisLabelInfo.DEFAULT_LABEL_PAINT, 
					false, 
		...
		{
			info[0] = new PeriodAxisLabelInfo(Week.class, 
	...
	private void runButtonPressed()
	{
		...
		for(DbComputation comp : compVector)
		{
			if (comp.hasGroupInput())
			{
				...
				if (compGroup == null)
				{
					...
					Logger.instance().debug3("Will execute with group " + groupCompName
						+ ", id=" + comp.getGroupId());
				}
				else if (compGroup.getGroupId() != comp.getGroupId())
				{
					showError("Multiple computations that are triggered from "
						...
						+ "in the GUI: computation '" + groupCompName
						+ "' uses group '" + compGroup.getGroupName()
						+ "', computation '" + comp.getName()
						+ "' uses group '" + comp.getGroupName()
						+ "'. -- Please de-select one of these computations to proceed.");
					return;
				}
				else
					Logger.instance().debug3("comp " + comp.getName()
						+ " also uses group id=" + comp.getGroupId());
			}
		}
		...
		if (compGroup != null)
		{
			try
			{
				...
				// Collect the transformed tsids in a hash set to remove any duplicates
				// that may result by transformation.
				TreeSet<TimeSeriesIdentifier> transformedTsids 
					= new TreeSet<TimeSeriesIdentifier>();
				for(Iterator<TimeSeriesIdentifier> tsidit = tsids.iterator(); 
					tsidit.hasNext(); )
				{
					...
					try
					{
						transformed = theDb.transformTsidByCompParm(tsid, firstInput, false, 
							false, "");
					}
					...
					Logger.instance().debug3("   original='" 
						+tsid.getUniqueString() + "'");
					Logger.instance().debug3("transformed='" 
						+transformed.getUniqueString() + "'");
					
					transformedTsids.add(transformed);
					Logger.instance().debug3("transformedTsids now has " + transformedTsids.size()
						+ " members.");
				}
...
Logger.instance().debug3("Setting selection list with " + transformedTsids.size() + " entries.");
				...
				for(Iterator<DbComputation> compit = compVector.iterator(); 
					compit.hasNext(); )
				{
					comp = compit.next();
					if (!comp.hasGroupInput())
					...
					for(TimeSeriesIdentifier tsid : groupTsIds)
					{
						try
						{
							DbComputation concrete = 
								DbCompResolver.makeConcrete(theDb, tsid, comp, true);
							concreteGroupComps.add(concrete);
						}
						catch(NoSuchObjectException ex)
						{
							errorMsgs.append("Cannot execute '" + comp.getName()
								+ "' for time series '" + tsid.getUniqueString()
								+ "': " + ex.getMessage() + "\r\n");
						}
					}
				}
			...
			{
				showError("Cannot expand group '" + compGroup.getGroupName()
					+ "': " + ex);
				return;
			}
		}
		...
		TeeLogger teeLogger = new TeeLogger(origLogger.getProcName(), origLogger, 
			traceLogger);
		...
		for(DbComputation comp : compVector)
		{
			...
			lowerBoundClosed = 
				TextUtil.str2boolean(comp.getProperty("aggLowerBoundClosed"));
			upperBoundClosed = 
				TextUtil.str2boolean(comp.getProperty("aggUpperBoundClosed"));
			...
			for(Iterator<DbCompParm> parmIt=comp.getParms(); parmIt.hasNext();)
			{
				...
				if (parm.isInput())
				{
					...
					{
						String msg = module + 
							" Exception filling input timeseries in " +
							"runButtonPressed() "+ex;
						showError(msg);
			...
			try
			{
				Logger.instance().info("Running computation " + comp.getName() 
					+ " modelRunId=" + comp.getModelRunId() + ", with parms: ");
				for(Iterator<DbCompParm> dcpit = comp.getParms(); dcpit.hasNext(); )
				{
					DbCompParm dcp = dcpit.next();
					CTimeSeries cts = runme.getTimeSeries(dcp.getSiteDataTypeId(), 
						dcp.getInterval(), dcp.getTableSelector(), comp.getModelRunId());
					TimeSeriesIdentifier tsid = cts != null ? cts.getTimeSeriesIdentifier() : null;

					Logger.instance().info("   " + dcp.getRoleName() + ":sdi=" +
						dcp.getSiteDataTypeId() + ",intv=" + dcp.getInterval()
						+ ",tsel=" + dcp.getTableSelector() + ",modId=" + dcp.getModelId()
						+ (cts==null ? " no existing TimeSeries" : 
							" Existing TS with " + cts.size() + " values in it ")
			...
			{
				//e.printStackTrace();
				showError(module + " DbCompException in " +
						"runButtonPressed() "+e.getMessage());
				continue;
			}
			catch (DbIoException e)
			{
				showError(module + " DbIOException in " +
						"runButtonPressed() "+e.getMessage());
				continue;
			}
			catch(NoSuchObjectException e)
			...
			for(DbCompParm parm : outputParms)
			{
				...
				for(CTimeSeries cts : runme.getAllTimeSeries())
					if (cts.getSDI() == parm.getSiteDataTypeId()
					 ...
					 && TextUtil.strEqual(cts.getTableSelector(), 
							parm.getTableSelector()))
					{
						outputs.add(cts);
						Logger.instance().info("After running, Found output sdi=" + cts.getSDI()
								+ ", size=" + cts.size() + ", units=" + cts.getUnitsAbbr());
						found = true;
						break;
					}
				if (!found)
					Logger.instance().info("No time series found for output role " 
						+ parm.getRoleName());
			}
		}
		...
		for(CTimeSeries cts:outputs)
		{
			...
			try
			{
				...
				Logger.instance().info("After fill - Output TS: " + cts.getDisplayName() 
						+ ", nsamps=" + cts.size() + ", units=" + cts.getUnitsAbbr());
			}
			catch (DbIoException e)
			{
				Logger.instance().warning(module + 
						" DbIoException in " +
						"runButtonPressed() filling outputs " + e.getMessage());
				continue;
			}
			catch (BadTimeSeriesException e)
			{
				Logger.instance().warning(module + 
						" BadTimeSeriesException in " +
						"runButtonPressed() filling outputs " + e.getMessage());
				continue;
			}
			finally
	...
	/**
	 * This is used when this GUI is launch from the 
	 * Comp Edit GUI
	...
	private void saveButtonPressed()
	{
		...
		{
			JOptionPane.showMessageDialog(this, "No computation output !", frameTitle, JOptionPane.WARNING_MESSAGE);
			return;
		}
		
		int r = JOptionPane.showConfirmDialog(this, "Are you sure to save the computation output ?", frameTitle, JOptionPane.YES_NO_OPTION);
	...
	private void saveCompOutput()
	{
		...
		try
		{
			for(CTimeSeries myseries:myoutputs)
			{
				...
				{
					String msg = module + " Can not write Time Series to the "
	...
	private void selectButtonPressed()
	{
		if(theDb!=null)
		{
			...
			{
				computationsListDialog = 
					new ComputationsListDialog(this, theDb,getRunCompFrametester());
			}
	...
	{
		JFrame myframe = new RunComputationsFrame(true);
	...
	/**
	 * @param runCompFrametester the runCompFrametester to set
	 */
...
class ComputationsTable extends JTable
{
	...
	public String getColumnName(int column)
	{
		if(column==0)
		{
			return RunComputationsFrame.computationName;
		}
		else if(column==1)
		{
			return RunComputationsFrame.description;
		}
		else
		{
			return "";
...
class ComputationsListDialog extends JDialog
{
	...
	{
		super(owner,RunComputationsFrame.selectFromListLabel,true);
		this.mytester = frameTester;


		boolean noCompToken  = mytester.getNoCompFilterToken();
		compListPanel=new ComputationsListPanel(mydb, 
				!noCompToken, true, owner);
		...
		JButton okButton = 
			new JButton(RunComputationsFrame.okButtonLabel);//"  OK  "
		JButton cancelButton = 
			new JButton(RunComputationsFrame.cancelButtonLabel);
	...
	/**
	 * @param mytester the mytester to set
	 */
