...
//	private Variable parseAsciiZeroLength(byte[] field)
//	{
...
//		}
	...
	private Variable pseudoBinToHex(byte[] field)
			throws FieldParseException
		{
			...
			return new Variable(hex.toUpperCase());
}
			...
			throws FieldParseException
	{
		...
		DecimalFormat dFormat = new DecimalFormat("0.000");
		//first 10 characters are type and id 
		for(fieldCount=0; fieldCount<11;fieldCount++)
		{
			if(fieldCount>2)
				result += (char)field[fieldCount];
		}
		...
		result = result + "       " + pbChar;
		
		//the rest of the first header
		for(fieldCount = 19; fieldCount < 41; fieldCount++)
		{
			String temp = "";
			temp += (char)field[fieldCount];
			temp += (char)field[++fieldCount];
			...
			if(fieldCount==22)
				for(int j = intString.length(); j<5; j++)
					blank += " ";
			if(fieldCount==24||fieldCount==26||fieldCount==28||fieldCount==30||fieldCount==32)
		...
		for(fieldCount = 42; fieldCount < 52; fieldCount++)
		{
			String temp = "";
			temp += (char)field[fieldCount];
			temp += (char)field[++fieldCount];
		...
		for(fieldCount = 53; fieldCount < 80; fieldCount++)
		{
			String temp = "";
			temp += (char)field[fieldCount];
			temp += (char)field[++fieldCount];
			if(fieldCount>59 && fieldCount<66 || fieldCount>71)
				temp+= (char)field[++fieldCount];
			int number=0;
			if(fieldCount == 56 || fieldCount == 58)
		...
		for(fieldCount = 81; fieldCount < 113; fieldCount++)
		{
			String temp = "";
			temp += (char)field[fieldCount];
			temp += (char)field[++fieldCount];
		...
		//data processing 
		for(fieldCount = 113; fieldCount < field.length-2; fieldCount++)
		{
			result += "\n";
			String bin = "";
			bin += (char)field[++fieldCount];
