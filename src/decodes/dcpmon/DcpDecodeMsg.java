...
public class DcpDecodeMsg
{
	 ...
	 * to the cgi
	 */
	public static String decodeData(XmitRecord xr, Date timestamp,
			DcpAddress dcpAddress)
	{
		...
		if (testing)
		{
			...
			if (decMsg == null || decMsg.equals(""))
				return wrapString(new String(xr.getRawMsg()));
			else
				return decMsg;
		}
	...
	private static synchronized String doDecoding(
		XmitRecord xr, Date timestamp, DcpAddress dcpAddress)
	{
		String module = "DcpDecodeMsg";
		String errorMsg = "Can not display message ";

		byte []msgData = xr.getRawMsg();
