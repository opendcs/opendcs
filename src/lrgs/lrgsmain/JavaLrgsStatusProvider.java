	...
	implements LrgsStatusProvider
{
	...
	public JavaLrgsStatusProvider(LrgsMain lrgsMain)
	{
		...
		try { lsse.hostname = InetAddress.getLocalHost().getHostName(); }
		catch(Exception e) { lsse.hostname = "Unknown"; }
...
//			System.out.println("setiing zero");
			...
			{
//				System.out.println("setiing everything zero");
	...
	private void syncDownlinks()
	{
		for(int i=0; i<lsse.maxDownlinks; i++)
		{
			...
			if (lii != null)
			{
//				System.out.println("name : "+lii.getInputName()+" type: "+lii.getType());
	...
	public int getAllClients(AttachedProcess allClients[])
	{
		...
		try
		{
				...
				i < allClients.length && it.hasNext(); i++)
			{
				...
				int idleSeconds = (int)(System.currentTimeMillis() / 1000L)
					- ap.lastPollTime;
				ap.stale_count = (short)
					(idleSeconds / (LddsParams.ServerHangupSeconds/4));
				ap.status =
					ap.stale_count == 0 ? "running" : ("stale-"+ap.stale_count);
			}
		}
