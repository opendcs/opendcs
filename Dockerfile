...
COPY --from=builder /app/java/opendcs-rest-api/build/libs/*.war /usr/local/tomcat/webapps/api.war
COPY --from=builder /app/javascript/opendcs-web-ui-react/build/libs/*.war /usr/local/tomcat/webapps/ROOT.war
COPY /docker_files/tomcat/conf/context.xml /usr/local/tomcat/conf/Catalina/localhost/api.xml
