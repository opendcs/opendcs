...
public class BaseApiIT extends AppTestBase
{
	...
	{
		String url = "org/opendcs/odcsapi/res/it/" + configuration.getName() + "/" + filename;
		log.info("Trying resource from {}", url);
	...
	String getJsonFromResource(String filename) throws Exception
	{
		try(InputStream implInputStream = getClass().getClassLoader()
				.getResourceAsStream("org/opendcs/odcsapi/res/it/" + configuration.getName() + "/" + filename))
		{
	...
	JsonPath getJsonPathFromResource(String filename)
	{
		URL resource = getClass().getClassLoader()
				.getResource("org/opendcs/odcsapi/res/it/" + configuration.getName() + "/" + filename);
		if(resource == null)
		{
	...
	{
		return configuration.getDecodesDatabase().getDbIo();
	}
	...
	public void deleteScheduleEntryStatus(DbKey statusId) throws DatabaseException
	{
		try (ScheduleEntryDAI dai = getDbIo().makeScheduleEntryDAO())
		{
	...
	public void deletePlatformStatus(DbKey statusId) throws DatabaseException
	{
		try (PlatformStatusDAI dai = getTsdb().makePlatformStatusDAO())
		{
	...
	public String getOrganization()
	{
		try
		{
			return configuration.getOpenDcsDatabase()
