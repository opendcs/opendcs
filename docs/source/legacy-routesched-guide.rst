...
.. note::
   The ``usgs-waterdata`` Data Source is only available when OpenDCS is
   built and run on Java 11 or later. The underlying ``usgs-waterdata-api``
   library requires Java 11; on a Java 8 build the class is not compiled
   into ``opendcs.jar`` and selecting the ``usgs-waterdata`` Data Source
   Type will fail at runtime.
