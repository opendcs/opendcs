...
public class SourceEditPanel extends DbEditorTab implements ChangeTracker, EntityOpsController
{
    ...
    */
    void addMemberButton_actionPerformed(ActionEvent e)
    {
        ...
        for(Iterator<DataSource> it = dataSource.groupMembers.iterator(); it.hasNext(); )
        {
    ...
    */
    ...
    public boolean saveChanges()
    {
        ...
        for(Iterator<RoutingSpec> it = getDb().routingSpecList.iterator(); it.hasNext(); )
        {
            ...
            if (rs.dataSource != null)
            {
                for(DatabaseObject dbo : affectedItems)
                {
                    if (dbo instanceof DataSource
                     && rs.dataSource.getName().equalsIgnoreCase(((DataSource)dbo).getName())  )
                    {
                        affected = true;
