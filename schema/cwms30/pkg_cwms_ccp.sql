...
create or replace package body cwms_ccp as
  ...
  */
    ...
    if upper(p_inout_flag) = 'IN' then
      ...
      for r1 in
        (select cc.computation_id,cc.group_id,tgp.db_office_code,ctp.site_datatype_id,
           ...
           nvl(ctp.interval_abbr, '*') interval,
  ...
  begin
    ...
    loop
      ...
      then
        ...
        delete from cp_comp_depends where computation_id = r1.computation_id and ts_id = p_ts_code;
        insert into cp_comp_depends(computation_id,ts_id) values(r1.computation_id, p_ts_code);
        commit;
      end if;
  ...
  is
  begin
    delete from cp_comp_depends where ts_id = p_ts_code;
    ...
    delete from tsdb_group_member_ts where ts_id = p_ts_code;
    commit;
  ...
  begin
    for r2 in
      (select distinct cc.loading_application_id
         from cp_comp_depends cd, cp_computation cc
         where cd.ts_id = p_ts_code_old and cc.enabled = 'Y'
           and cc.loading_application_id is not null and cd.computation_id = cc.computation_id
      )
    loop
      insert into cp_comp_tasklist(record_num,loading_application_id,site_datatype_id,
    ...
    update cp_comp_depends set ts_id = p_ts_code where ts_id = p_ts_code_old;
    update cp_comp_ts_parm set site_datatype_id = p_ts_code where site_datatype_id = p_ts_code_old;
    update tsdb_group_member_ts set ts_id = p_ts_code where ts_id = p_ts_code_old;
    commit;
  ...
  begin
    ...
    loop
      update cp_comp_ts_parm set interval_abbr = l_interval,
