...
GRANT ALTER ANY TABLE,CREATE ANY TABLE,CREATE ANY INDEX,CREATE ANY SEQUENCE,
  ...
  CREATE PUBLIC SYNONYM,DROP PUBLIC SYNONYM, CREATE ANY ROLE, CREATE ANY USER,
  TO OTSDB_ADMIN;

CREATE USER "${TSDB_ADM_SCHEMA}" IDENTIFIED BY "${TSDB_ADM_PASSWORD}"
...
begin
	...
	loop
		...
		stmt := 'CREATE TABLE TS_NUM_' || tbl_name || q'[(
			TS_ID INT NOT NULL,
			SAMPLE_TIME NUMBER(19) NOT NULL,
			TS_VALUE DOUBLE PRECISION NOT NULL,
			-- Bitwise flags for each value
