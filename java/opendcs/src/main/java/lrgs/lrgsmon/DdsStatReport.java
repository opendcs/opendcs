...
public class DdsStatReport
{
 		...
 		{	// Parse assuming user enter HH (hour)
 			...
 			{	// if we get here, try parsing without the HH
				...
				catch (ParseException ex2)
				{
					...
					log.atError()
					   .setCause(ex2)
					   .log("Cannot parse the following input time '{}' -- aborting. Use Format for " +
	...
	private void readDdsConnectionInfo()
	{
		...
		{
			String msg = module + " Cannot read DB auth from configuration '" + authFileName;
		...
		{
			log.atError().setCause(ex).log("Cannot interface with LRGS database.");
			System.exit(1);
		}
	}
	...
	private void loadLrgsDBConfiguration(String lrgsConfigFilePath)
	{
		...
		{
			log.atError().setCause(ex).log("Cannot load the LRGS database config file.");
			System.exit(1);
		}
	}
}