...
public class CmdUser extends LddsCommand
{
	...
	*/
		...
		throws ArchiveException, IOException
	{
		...
		{
			log.warn("User {} attempted user-admin command when not authenticated -- rejected.", ldds.getName());
		...
		{
			throw new ArchiveException("Unexpected Exception '" + subcmd + "'",
									   LrgsErrorCode.DDDSINTERNAL, false, ex);
		}
	}
	...
	private void listDir(boolean local, StringBuilder sb)
	{
		...
		{
			LoggingEventBuilder le = local ? log.atDebug() : log.atWarn();
			le.setCause(ex).log("Cannot list user dir '{}'", dir.getPath());
			return;
		}
		if (ls == null || ls.length == 0)
		{
			LoggingEventBuilder le = local ? log.atDebug() : log.atWarn();
			le.log("User dir '{}' is empty.", dir.getPath());
			return;
		}
//TODO handle this
	...
	private void addUserToBuffer(PasswordFileEntry pfe, StringBuilder sb, boolean local)
	{
		...
		{
			log.atTrace().setCause(ex).log("unknown user.");
		}
		...
		throws ArchiveException, IOException
	{
		...
		if (dir.isDirectory())
		{
			...
			{
				log.warn("Cannot remove dir '{}'!", dir.getPath());
			}
			else
				actionsTaken++;
		}
		...
		throws ArchiveException, IOException
	{
		...
		else
		{
			...
			{
				log.atWarn().setCause(ex).log("Cannot decrypt password.");
				return 0;
			}
		}
		...
		try
		{
			...
			if (pfe == null)
			{
				...
				{ /* won't happen */
					log.atError().setCause(ex).log("Error processing user that shouldn't happen.");
				}
			}
		...
		catch(IOException ioe)
		{
			throw new LddsRequestException("Cannot write password file '"
				+ pf.getFile().getPath()
				+ "'", LrgsErrorCode.DDDSINTERNAL, true, ioe);
		}
		...
		throws ArchiveException, IOException
	{
		...
		catch(AuthException ex)
		{
			throw new ArchiveException(
				...
				LrgsErrorCode.DDDSAUTHFAILED, false, ex);
		}
	...
	private PasswordFile readPasswordFile(boolean local)
	{
		...
		if (lrgsDb != null)
		{
			...
			{
				log.atWarn().setCause(ex).log("Error reading DbPasswordFile.");
			}
			return dpf;
		}
		...
		catch(IOException ioe)
		{
			...
			catch(IOException ioe2)
			{
				...
				log.atWarn().setCause(ioe2).log("Cannot read password file '{}'", pfname);
				return pf;
			}
		}
		return pf;
