...
public class HDBRatingTable
{
   ...
   public HDBRatingTable (TimeSeriesDb tsdb,String type, DbKey indep_sdi)
   {
	...
	throws DbCompException
	{
		...
		{
			String msg = "Cannot find rating id for type " + ratingType + " sdi " + sdi;
		...
		else
		{
			...
			{
				String msg = "Cannot find effective dates for rating id " + ratingId;
				throw new DbCompException(msg, ex);
			}
		}
	}
	...
	{
		log.trace("Doing effRange: {} effStart: {} effEnd: {}", valueDate, effEndDate, effEndDate);

		if (valueDate !=null)
		{
			if (effStartDate !=null && valueDate.before(effStartDate))
			{
				log.trace("Doing effRange start: {}", valueDate.before(effStartDate));
				return false;
			}
			if (effEndDate !=null && (valueDate.equals(effEndDate) || valueDate.after(effEndDate)))
			{
				log.trace("Doing effRange end {}", (valueDate.equals(effEndDate) || valueDate.after(effEndDate)));
				return false;
			}
		}
		return true;
	...
	throws DbCompException
	{
		...
		String idString = " ratingId=" + ratingId+ ", indepValue=" + indepValue;
		log.trace("Performing rating: '{}'", idString);
		...
		{
			String msg = "SQL Error performing rating for rating" + idString;
			throw new DbCompException(msg, ex);
		}
