...
public class CwmsTimeSeriesDb extends TimeSeriesDb
{
		...
		throws BadConnectException
	{
		...
		if( pool != null)
		{
			try(Connection conn = pool.getConnection();)
			{
				...
				{
					log.atWarn().setCause(ex).log("Exception in hec.data.Units.getAvailableUnits");
				}
				...
				{
					log.atError().setCause(ex).log("Cannot load baseParam Units Map");
				}
		...
		throws DbIoException, NoSuchObjectException
	{
		...
		try
		{
			...
			if (rs != null && rs.next())
			{
				...
				throw new NoSuchObjectException("No Time Series with specified identifiers.");
		...
		{
			throw new DbIoException("Unable to Set Parameter information", ex);
		}
		finally
		{
			dataTypeDao.close();
		...
		throws DbIoException, NoSuchObjectException, BadTimeSeriesException
	{
		...
		if (transformed)
		{
			...
			log.trace("CwmsTimeSeriesDb.transformTsid origString='{}', new string='{}', parm={}",
	...
	public String[] getParamTypes() throws DbIoException
	{
		...
		{
			retStr = null;
			throw new DbIoException("Unable to retrieve CWMS Parameter Types.", ex);
		}
		return retStr;
	}
		...
		throws DbIoException
	{
		...
		{
			throw new DbIoException("Unable to query CWMS Parameter types", ex);
		}
		finally
		{
			dao.close();
		...
		throws DbIoException
	{
		...
		{
			throw new DbIoException("Unable to retrieve CWMS Version (TimeSeries name) values", ex);
		}
		finally
		{
			dao.close();
	...
	public Connection getConnection()
	{
		...
		{
			log.error("CwmsTimeSeriesDb.getConnection -- loginInfo is null! DB not initialized?");
