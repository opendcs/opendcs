...
class CheckOperation extends DecodesOperation
{
	...
	private static final Marker MODULE = MarkerFactory.getMarker("Check");
	...
	*/
		...
		throws DecoderException
	{
		...
		case CHECK_FOR_NUMBER:
			...
			log.trace(MODULE, "check(number), pos={}, result={}", pos, found);
			break;
		case CHECK_FOR_SIGN:
			found = dd.checkSign();
			log.trace(MODULE, "check(sign), pos={}, result={}", pos, found);
			break;
		case CHECK_FOR_STRING:
			found = dd.checkString(checkString);
			log.trace(MODULE, "check('{}'), pos={}, result={}", checkString, pos, found);
			break;
		}
