...
public class AsciiSelfDescFunction extends DecodesFunction
{
	...
	// by using an SLF4j marker, the trace dialog will be able to focus in on decodes script issues
	// Another option, which may be better as it's set per thread and outside the individual classes
	// is the MappedDiagnosticContext feature.
	...
	public int parse(String msgData, DecodedMessage decmsg)
	{
			...
			blockMatcher = sensorBlockPat.matcher(parseData))
		{
			...
			log.trace(MODULE, "Sensor Block at {}, block extent=({},{}): '{}'",
			...
			log.trace(MODULE, "Sensor Label '{}'", label);
			int moff = -1;
			...
			log.trace(MODULE, "Minute Interval '{}'", smint);
			...
			log.trace(MODULE, "Sensor Data '{}'", sensorData);
			TimeSeries ts = null;
			int sensorNumber = -1;
			if (decmsg != null)
			{
				...
				if (ts == null)
					log.warn(MODULE, "Cannot map sensor for label '{}' -- values will be discarded.", label);
				else
				{
					...
					if (processMOFF && moff > 0)
					{
						...
						log.trace(MODULE, "After Minute OFFset {}, timer={}", moff, timeStamp);
					}
					if (mint != -1)
					{
						log.trace(MODULE, "Setting interval for sensor {} to {} minutes.", sensorNumber, mint);
			...
			while(sampleMatcher.find())
			{
				...
				log.trace(MODULE,"tsample[{}]: {}",  numSamples++, sample);
				if (ts != null)
				{
			...
			if (!sampleFound)
			{
				log.trace(MODULE, "No samples found.");
			}
		}
		...
		if (blockMatcher.find())
		{
			// Get the label, minute offset, minute index and sensor data
			numBlocks++;
			log.trace(MODULE, "Battload block at ({},{}): '{}'",
			...
			log.trace(MODULE, "Battload Sensor Label '{}' value={}", label, sample);
			if (decmsg != null)
				{
		...
		log.trace(MODULE, "Processed {} blocks.", numBlocks);
		...
		throws DecoderException
	{
		...
		log.trace(MODULE, "Processing '{}'", toParse);
