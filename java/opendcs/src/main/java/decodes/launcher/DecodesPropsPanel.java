    ...
    private void dbPasswordButtonPressed()
    {
        try
        {
            ...
            if (dlg.isOK())
            {
                ...
                {
                    String msg = labels.getString("DecodesPropsPanel.cannotSavePassErr");
                    log.atError()
                       .setCause(ex)
                       .log(LoadResourceBundle.sprintf(msg,afn));
                       
                    parent.showError(LoadResourceBundle.sprintf(msg,afn) + ex);
                }
            }
        }
        catch (AuthException ex)
