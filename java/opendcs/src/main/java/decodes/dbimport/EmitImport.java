...
public class EmitImport
{
	...
	*/
	public void parseCS(String line)
	{
		...
		{
			warning("{}Config Sensor skipped - cannot parse sensor number", ex);
			return;
		}
	...
	*/
	public void parseCA(String line)
	{
		...
		{
			warning("CA record skipped - cannot parse sensor number", ex);
			return;
		}
		ConfigSensor cs = platformConfig.getSensor(sensorNum);
		if (cs == null)
	...
	*/
	public void parseCF(String line)
	{
		...
		if (lab.length() == 0)
		{
			...
			{
				failure("CF (format statement) missing required label - skipped");
				return;
			}
		...
		if (s.length() > 0)
		{
			...
			else // platform.site already supplied by USGS number
			{
				...
				*/
				...
				{
					warning("SI1 record contains HB5 name that is already used by a different" +
	...
	*/
	public void parseSD(String line)
	{
		...
		if (platform == null)
		{
			warning("SD (Site Data) can only occur inside Site block - ignored");
			return;
		}
		if (line.length() < 20)
		{
	...
	*/
	public void parseSS(String line)
	{
		if (platform == null)
		{
			warning("SS (Site Sensor) record not inside a Site (SI...ES) block - ignored");
			return;
		}
		...
		{
			warning("Invalid recording interval '{}' -- ignored.", ex, fields[4]);
			recordingInterval = -1;
		}
		int timeOffset = -1;
		...
		if (fields.length >= 8)
		{
			...
			if (fields.length >= 9)
			{
				...
				if (cs != null && s.length() > 0)
				{
					...
					{
						log.atTrace().setCause(ex).log("Unable to set minimum property.");
					}
				}
	...
	*/
	public void parseSS1(String line)
	{
		if (platform == null)
		{
			warning("SS1 (Site Sensor Extension) record not inside a Site (SI...ES) block - ignored");
			return;
		}
		if (platform.getConfig() == null)
		{
			warning("SS1 (Site Sensor Extension) without corresponding Device Config - ignored");
			return;
		}
		...
		{
			failure("SS1 (Site Sensor Extension) with invalid sensor number - ignored");
			return;
		}
	...
	*/
	public void parseES(String line)
	{
		...
		{
			failure("A platform must have at least one transport medium (ST, RD, or EDL).");
			platform = null;
			return;
		}
