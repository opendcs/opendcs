...
public class HydrometDMS3Formatter extends OutputFormatter
{
    ...
    public HydrometDMS3Formatter()
    {
        ...
        if (!pdt.isLoaded())
        {
            log.info("Loading PDT. Will wait 6 second for file to load or download.");
            pdt.startMaintenanceThread(DecodesSettings.instance().pdtUrl,
                DecodesSettings.instance().pdtLocalFile);
            // Give it up to 6 seconds to load.
            long start = System.currentTimeMillis();
            while(System.currentTimeMillis() - start < 6000L && !pdt.isLoaded())
            {
                try
                {
                    Thread.sleep(1000L);
                ...
                {
                    log.atWarn().setCause(ex).log("Pdt Load wait interrupted.");
                }
            }
            log.info(pdt.isLoaded() ?
                     "Pdt Loaded Succesfully" :
                     "Pdt did not load in time, will continue attempts in background.");
        }
    }
