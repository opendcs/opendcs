...
public class EquipmentModelListIO extends SqlDbObjIo
{
	...
	*/
		...
		throws DatabaseException, SQLException
	{
			 ...
			 ResultSet rs = stmt.executeQuery( q );)
		{
			...
			if (eqm == null)
			{
				throw new DatabaseException("No equipment model found with ID " + id);
			}
			stmt.close();
			return eqm;
		}
	...
	*/
		...
		throws DatabaseException, SQLException
	{
		...
		{
			throw new DatabaseException("Unable to write equipment properties.", ex);
		}
		finally
		{
			propsDao.close();
	...
	*/
		...
		throws DatabaseException, SQLException
	{
		...
		{
			throw new DatabaseException("Unable to delete equipment properties.", ex);
		}
		finally
		{
			propsDao.close();
	...
	{
		try (Statement stmt = createStatement();
			 ResultSet rs = stmt.executeQuery("SELECT id FROM EquipmentModel where name = " + sqlReqString(name));)
		{
