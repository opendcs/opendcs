...
public class EdlPMParser extends PMParser
{
	...
	*/
	...
	public static final String POLL_STOP = "PollStop";
	...
	public void parsePerformanceMeasurements(RawMessage msg) throws HeaderParseException
	{
		...
		for (int p = 0; p < len - 3; p = e)
		{
			...
			if (data[p] == (byte) '/' && data[p + 1] == (byte) '/')
			{
				...
				// Removed uppercase as in case of dataloggers if transport
				// medium is defined in lowercase,
				...
				else if (s.startsWith("DEVICE"))
				{
					...
					if (hyphen >= 0)
					{
						if (space > 0 && hyphen < space)
							...
							val = val.substring(0, space) + "-" + val.substring(space + 1);
					}
					else
					// no hyphen
					{
				...
				else if (s.startsWith("EDL NOTES") || s.startsWith("PFC NOTES")
				...
				else if (s.startsWith("POLL START"))
				{
					try
					{
						Date d = endTimeSdf.parse(s.substring(10).trim());
						msg.setPM(POLL_START, new Variable(d));
					}
					catch (Exception ex)
					{
						log.atError().setCause(ex).log("Unparsable POLL START '{}': Ignored.", beginTime);
					}
				}
		...
		if (endTime != null)
		{
			...
			else
			{
				...
				for (; i < endTime.length() && i - idx <= 4 && Character.isDigit(endTime.charAt(i)); i++)
					;
				// i now points to first non-digit after TZ

				switch (i - idx)
				// i-idx is # of digits after sign.
				{
				case 0:
					endTime = endTime.substring(0, idx) + "0000";
					break;
				case 1: // 1 digit hour? move to position 2 in HHMM:
					endTime = endTime.substring(0, idx) + "0" + endTime.charAt(idx) + "00";
					break;
