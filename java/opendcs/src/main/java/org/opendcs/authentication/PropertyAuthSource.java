 ...
 *
 */
public class PropertyAuthSource implements AuthSource
{
    private Properties credentials = new Properties();

    public PropertyAuthSource(String configString) throws AuthException
    {
        for(String pair: configString.split(",")) {
            ...
            if (envValue == null)
            {
                throw new AuthException(String.format("Variable '%s' is not defined in this processes java properties.",envName));
            }
            credentials.setProperty(propName, envValue);
        }
        if (credentials.isEmpty())
        {
            throw new AuthException("Configuration for properties source empty or not parseable.");
        }
    }
