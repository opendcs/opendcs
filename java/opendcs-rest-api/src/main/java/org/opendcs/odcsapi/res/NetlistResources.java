...
public final class NetlistResources extends OpenDcsResource
{
    ...
    private static final WebAppException NO_NETWORKLIST_DAO =
        new WebAppException(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode(), "No NetworkList Dao is available.");
            ...
            throws WebAppException
    {
        ...
        catch (OpenDcsDataException ex)
        {
            throw new WebAppException(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode(),
                          "Unable to retrieve network lists",
                                        ex);
        }
    }

    static ApiNetlistRef mapRef(NetworkList nl)
    {
            ...
            throws WebAppException, OpenDcsDataException
    {
        ...
        try (var tx = db.newTransaction())
        {
            var nl = tx.wrapErrors(() -> dao.getById(tx, DbKey.createDbKey(netlistId)).orElse(null));
            if (nl == null)
            {
            ...
            throws WebAppException, OpenDcsDataException
    {
        ...
        try (var tx = db.newTransaction())
        {
            ...
            var ret = tx.wrapErrors(() -> dao.save(tx, nlList));
            return Response.status(Response.Status.CREATED).entity(map(ret)).build();
        }
            ...
            throws OpenDcsDataException, WebAppException
    {
        ...
        try (var tx = db.newTransaction())
        {
            return tx.wrapErrors(() ->
            {
                dao.delete(tx, DbKey.createDbKey(netlistId));
                return Response.noContent().entity("ID " + netlistId + " deleted").build();
            });
        }
        catch (OpenDcsDataException ex)
        {
            if (ex.getCause() instanceof OpenDcsConstraintException)
            {
                return Response.status(Response.Status.CONFLICT)
                                  .entity(" Cannot delete network list with ID " + netlistId
                                     + " because it is used by at least one routing spec"
                                ).build();
            }
            throw ex;
        }
