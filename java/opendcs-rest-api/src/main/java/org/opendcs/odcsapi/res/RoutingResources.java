...
public final class RoutingResources extends OpenDcsResource
{
    ...
    public Response getRoutingRefs() throws WebAppException
    {
        var db = createDb();
        ...
        catch (OpenDcsDataException ex)
        {
            throw new WebAppException(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode(),
                                      "Unable to retrieve routing spec references.",
                                      ex);
        }
    }

    static ApiRoutingRef mapRef(RoutingSpec rs)
    {
    ...
    @QueryParam("routingid") Long routingId) throws WebAppException
    {
        ...
        try (var tx = db.newTransaction())
        {
            return Response.ok()
                           .entity(
                                tx.wrapErrors(() -> dao.getById(tx, DbKey.createDbKey(routingId)))
                                // we only want to wrap the actual database errors here, this one
                                // is for this section of code to decide what to do.
                                  .map(rs -> map(rs))
                                  .orElseThrow(() ->
                                            new DatabaseItemNotFoundException(
                                            "No Routing Spec with id " + routingId + " exists in this database.")
                                  )
                           )
                           .build();
        }
        ...
        {
            throw new WebAppException(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode(),
                                      "Unable to retrieve routing spec.",
                                      ex);
        }
    }
